...
box_plot <- plot_ly(hri.emer, x = ~factor(year), y = ~emer.per.capita, type = 'box', boxpoints = "outliers", jitter = 0.3, pointpos = -1.8, outliercolor = "red", text = ~state, textposition = "bottom center")

# You can customize the appearance or add interactivity as needed
box_plot <- box_plot %>% layout(title = "Box Plot of Emergency Room Visits Per Capita Over Years")
...
```
```{r}
ggplot(hri.mort, aes(x = factor(year), y = mortalities.total)) +
  geom_boxplot(outlier.colour = "red", position = position_dodge(0.8)) +
  geom_text_repel(data = hri.mort %>% group_by(year) %>% filter(mortalities.total > quantile(mortalities.total, 0.75) + 1.5*IQR(mortalities.total) | mortalities.total < quantile(mortalities.total, 0.25) - 1.5*IQR(mortalities.total)),
                  aes(label = state),
                  box.padding = 0.5, show.legend = FALSE) +
  labs(title = "Box Plot of Mortality Per Capita Over Years") +
  theme_minimal()
...
ggplot(hri.mort, aes(x = factor(year), y = mortalities.total)) +
  ...
  geom_text_repel(data = hri.mort %>% group_by(year) %>% filter(mortalities.total > quantile(mortalities.total, 0.75) + 1.5*IQR(mortalities.total) | mortalities.total < quantile(mortalities.total, 0.25) - 1.5*IQR(mortalities.total)),
                  aes(label = state),
                  box.padding = 0.5, show.legend = FALSE, max.overlaps = Inf) +
  labs(title = "Box Plot of Mortality Per Capita Over Years") +
  theme_minimal()
```
